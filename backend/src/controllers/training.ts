...
export async function createRole(req: AuthRequest, res: Response): Promise<void> {
  try {
    ...
    if (!name || !sourceIds || !Array.isArray(sourceIds)) {
      res.status(400).json({ error: 'Name and sourceIds array are required' });
      return;
    }
 ...
 */
export async function getRoleKnowledge(req: AuthRequest, res: Response): Promise<void> {
  try {
    const userId = req.userId;
    if (!userId) {
      res.status(401).json({ error: 'Unauthorized' });
    ...
    if (knowledge === null) {
      res.status(404).json({ error: 'Role not found or no sources' });
      return;
    }
